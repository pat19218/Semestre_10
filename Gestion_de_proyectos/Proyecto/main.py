...
class Registro(Frame):
    ...
    def create_widgets(self):
        ...
        Label(self.frame2, text='Agregar Nuevos Datos', fg='white', bg='navy', font=('Rockwell', 12, 'bold')).grid(
            columnspan=2, column=0, row=0, pady=5)
        ...
        Label(self.frame2, text='DPI', fg='white', bg='navy', font=('Rockwell', 13, 'bold')).grid(column=0, row=3, pady=15)
        Label(self.frame2, text='INCIDENCIA', fg='white', bg='navy', font=('Rockwell', 13, 'bold')).grid(column=0, row=4,
                                                                                                       pady=15)
        Label(self.frame2, text='DESCRIPCION', fg='white', bg='navy', font=('Rockwell', 13, 'bold')).grid(column=0, row=5,
                                                                                                         pady=15)
        ...
        Entry(self.frame2, textvariable=self.DPI, font=('Arial', 12)).grid(column=1, row=3)
        Entry(self.frame2, textvariable=self.INCIDENCIA, font=('Arial', 12)).grid(column=1, row=4)
        Entry(self.frame2, textvariable=self.DESCRIPCION, font=('Arial', 12)).grid(column=1, row=5)

        Label(self.frame4, text='Control', fg='white', bg='black', font=('Rockwell', 12, 'bold')).grid(columnspan=3, column=0,
                                                                                                    row=0, pady=1, padx=4)
        Button(self.frame4, command=self.agregar_datos, text='REGISTRAR', font=('Arial', 10, 'bold'), bg='magenta2').grid(
            column=0, row=1, pady=10, padx=4)
        Button(self.frame4, command=self.limpiar_datos, text='LIMPIAR', font=('Arial', 10, 'bold'), bg='orange red').grid(
            column=1, row=1, padx=10)
        Button(self.frame4, command=self.eliminar_fila, text='ELIMINAR', font=('Arial', 10, 'bold'), bg='yellow').grid(
            column=2, row=1, padx=4)
        Button(self.frame4, command=self.buscar_nombre, text='BUSCAR POR NOMBRE', font=('Arial', 8, 'bold'),
               bg='orange').grid(columnspan=2, column=1, row=2)
        ...
        Button(self.frame4, command=self.mostrar_todo, text='MOSTRAR DATOS DE MYSQL', font=('Arial', 10, 'bold'),
               bg='green2').grid(columnspan=3, column=0, row=3, pady=8)
        ...
        self.tabla['columns'] = ('Nombre', 'DPI', 'INCIDENCIA', 'DESCRIPCION')

        self.tabla.column('#0', minwidth=100, width=120, anchor='center')
        self.tabla.column('Nombre', minwidth=100, width=130, anchor='center')
        self.tabla.column('DPI', minwidth=100, width=120, anchor='center')
        self.tabla.column('INCIDENCIA', minwidth=100, width=120, anchor='center')
        self.tabla.column('DESCRIPCION', minwidth=100, width=105, anchor='center')

        self.tabla.heading('#0', text='Codigo', anchor='center')
        self.tabla.heading('Nombre', text='Nombre', anchor='center')
        self.tabla.heading('DPI', text='DPI', anchor='center')
        self.tabla.heading('INCIDENCIA', text='INCIDENCIA', anchor='center')
    ...
    def agregar_datos(self):
        ...
        DPI = self.DPI.get()
        INCIDENCIA = self.INCIDENCIA.get()
        DESCRIPCION = self.DESCRIPCION.get()
        datos = (nombre, DPI, INCIDENCIA, DESCRIPCION)
        if codigo and nombre and DPI and INCIDENCIA and DESCRIPCION != '':
            self.tabla.insert('', 0, text=codigo, values=datos)
            self.base_datos.inserta_producto(codigo, nombre, DPI, INCIDENCIA, DESCRIPCION)
