...
# Se aplicara 80% entrenamiento 20% validacion
# Se crea el set de entrenamiento  

L = 64000
X_train = nnInput[:L,:]
Y_train = nnInput[1:L+1,:] #se puede usar nnOutput[:L,:]

#Se crea el set de test
X_test = nnInput[L:-1,:]
Y_test = nnInput[L+1:,:]  #se puede usar nnOutput[L:-1,:]
...
nn = [128,128,128,128] #cantidad de neuronas valor visto 128 todas
...
layer2 = tf.keras.layers.Dense(nn[1], activation='elu')
layer3 = tf.keras.layers.Dense(nn[2], activation='relu')
layer4 = tf.keras.layers.Dense(nn[3], activation='elu')
layer5 = tf.keras.layers.Dense(3)
...
# Graficar comparación entre simulación numérica e inferencia de la red neuronal en un solo subplot 3D

fig = pyplot.figure()
ax = fig.add_subplot(111, projection='3d')

# Trayectoria real obtenida mediante integración numérica
ax.plot(x_sim, y_sim, z_sim, linewidth=1, label='Simulación numérica')
ax.scatter(x0[0, 0], x0[0, 1], x0[0, 2], color='r')

# Trayectoria predicha por la red neuronal
ax.plot(nnTrajectory[:, 0], nnTrajectory[:, 1], nnTrajectory[:, 2], linewidth=1, linestyle='dashed', label='Inferencia NN')
ax.scatter(x0[0, 0], x0[0, 1], x0[0, 2], color='r')
