...
# creacion del Json que vamos a manipular
#dato = {"id":0,"tx":0.0, "ty":0.0, "tz":0.0, "rx":0.0, "ry":0.0, "rz":0.0}
...
while(True):
    ...
    for tag in img.find_apriltags(fx=f_x, fy=f_y, cx=c_x, cy=c_y): # defaults to TAG36H11
        ...
        # Obtener información de posición y rotación del tag
        tag_id = tag.id()
        ...
        mensaje = uart.readchar() # espero a que ESP32 me envie un Json
        print(mensaje)

        if  mensaje == 65:
            uart.write("z" + str(tag_id) + "a" + str(tx) + "b" + str(ty) + "c" + str(tz) + "d" + str(rx) + "e" + str(ry) + "f" + str(rz) + "g")
            print("eeennnnnviiiiaaaaadooooo")
            #time.sleep(0.5)
            sensor.skip_frames(time=200)
    print("Los FPS: " + str(clock.fps()))
    #print(dato)
